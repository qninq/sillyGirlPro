...
`GET /api/admin/tasks` 返回的任务包含 `last_run`（上次执行时间戳，内存态、重启后清空，手动运行与定时触发都会记录）与 `next_run`（下次执行时间戳，按任务 Cron 表达式推算；停用任务不计算）。

定时任务的执行语义：
1. 「触发命令」选择了已安装脚本（`node xxx.js` / `python xxx.py`）→ 定时把触发口令传给该脚本，脚本 `s.reply()` 送达接收人；
2. 未选择脚本但配置了接收人 → 触发口令**当作消息走正常规则匹配管线**：命中系统内置指令或插件规则（如 `time`、`版本`）即触发对应命令，回复送达接收人；未命中任何规则则不发送，仅记日志说明；
3. 既无脚本也无接收人 → 记警告跳过。

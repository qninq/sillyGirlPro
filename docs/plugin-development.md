...
- **左侧目录栏**：顶部为「脚本目录」标题（右侧显示总数徽章）与筛选搜索框；下方每个语言分类一张可折叠卡片——卡片标题行含分类名、脚本数量徽章（有脚本时蓝色高亮）与旋转箭头，展开后卡片内列出该分类下的脚本（名称 + 版本号 + 启用/停用徽章）。点击脚本名称打开编辑器；点击「启用/停用」徽章即时切换已加载脚本运行状态。
- **右侧编辑区**：工具栏含语言标签、编辑器黑/白主题切换，以及「新建 / 配参 / 调试 / 保存 / 更多（删除）」操作；编辑器随窗口占满可用高度，代码超长时在编辑器内部滚动。
...
- 「新建」选择语言分类并填写初始源码；JS/Python 系脚本沿用元数据校验（`[title]`/`[name]`/`[desc]`/`[version]` 及 `[on_start]`/`[web]`/`[cron]` 之一，消息响应脚本可用 `[rule]`）。

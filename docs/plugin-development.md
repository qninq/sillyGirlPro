...
`[on_start]` / `[web]` 插件以后台进程方式常驻运行。后台进程**非零退出（崩溃）**时系统会自动重新拉起：退避间隔 5 秒 → 10 秒 → 20 秒，最多连续 3 次；进程稳定运行 60 秒后计数清零。连续失败超过上限会停止重试，并通过其余在线平台向各平台 masters 推送告警、同时向管理员邮箱发一封告警邮件。手动重载插件会立即清空失败计数。正常退出（`exit 0`，如插件主动结束监听）不会触发重启。
...
Admin 面板「消息工具-转发」按平台、群号匹配来源群消息后，**直接转发**到配置的「转发目标」，无需编写脚本：

- 来源支持群聊（按群号匹配）和单聊会话（无群号时按会话/用户 ID 匹配，如 Web 会话 rid、C2C 用户 openid）。
- 每条目标可选 **群聊**（按群号推送，QQ 官方机器人填 `group_openid`）或 **私聊**（按用户 openid 推送）。
- 支持同时配置多个目标（可跨平台）；目标与来源相同时自动跳过，避免回环。
- QQ 官方机器人的群主动消息受平台月度配额限制，私聊主动推送需要对方先私聊过机器人。

如需自定义处理（过滤、改写、多步逻辑），仍可编写 `@carry` 处理脚本，消息会交给脚本自行决定是否回复或转发：
...
回复内容支持 CQ 媒体码，适配器会按平台能力发送：

```js
s.reply(`看这张图[CQ:image,url=https://example.com/a.png]`)
s.reply(`[CQ:video,url=https://example.com/a.mp4]`)
```
...
管理后台的「插件开发」页面（`/admin/scripts`，侧边栏位于插件市场下方）是一个本地脚本 IDE，面向**本地独立应用脚本**——启动任务（`[on_start]`）、常驻后台服务（`[web]`）、定时任务（`[cron]`）等不响应消息规则的脚本，也包含普通消息响应脚本。与本地插件共用 `<数据目录>/plugins/local/` 存储，新建的可执行脚本进入插件加载链路，插件市场页同步可见。

### 界面布局

- **左侧目录栏**：顶部为「脚本目录」标题（右侧显示总数徽章）与筛选搜索框；下方每个语言分类一张可折叠卡片——卡片标题行含分类名、脚本数量徽章（有脚本时蓝色高亮）与旋转箭头，展开后卡片内列出该分类下的脚本（名称 + 版本号 + 启用/禁用徽章）。点击脚本名称打开编辑器；点击「启用/禁用」徽章即时切换已加载脚本运行状态。搜索时命中分类自动展开并只显示有结果的分类，数量徽章实时显示命中数；清空搜索后恢复原有折叠状态。
- **右侧编辑区**：工具栏含语言标签、编辑器黑/白主题切换，以及「新建 / 配参 / 调试 / 保存 / 更多（删除）」操作；编辑器随窗口占满可用高度，代码超长时在编辑器内部滚动。编辑器底部有状态栏，显示语言、来源（AutMan 插件/本地文件）、脚本文件路径与「N 行 M 字符」统计，路径超长自动省略，随编辑器黑/白主题同色。

### 语言分类

共 8 类，按源码内容启发式归类：ES5、Node.js、TypeScript、Python、Golang、Adapter JS、Adapter Python、Adapter Go。仅 JS/Python 系分类可执行（支持保存、配参、在线调试）；TypeScript/Golang/Adapter Go 为不可执行占位分类，可创建、编辑、保存 `.ts`/`.go` 文件，不支持在线调试。

### 新建与调试

- 「新建」复用插件市场「新增插件」的完整编辑器弹窗：脚本模式下标题为「新建应用脚本」，语言下拉可选全部 8 种分类（预填对应语言模板），切换语言时若内容仍为初始模板则自动替换（已手动编辑则保留），保存走应用脚本接口——JS/Python 系脚本沿用元数据校验（`[title]`/`[name]`/`[desc]`/`[version]` 及 `[on_start]`/`[web]`/`[cron]`/`[module]` 之一，消息响应脚本可用 `[rule]`），TypeScript/Golang/Adapter Go 占位脚本仅落盘不加载；弹窗内可切换编辑器黑/白主题并格式化 JS 代码。
- 「配参」与插件市场共用同一套插件配置表单弹窗。
- 「调试」复用插件运行时装配（Node 预加载/依赖、Python 模块、gRPC 环境变量）执行一次脚本，stdout/stderr 经 SSE 流式推送，结束显示退出码与耗时；默认 120 秒超时（可通过 `timeout` 查询参数调整，上限 600 秒），超时或页面断开自动终止进程。
